...
pub type WiFiRunner = cyw43::Runner<'static, Output<'static>, PioSpi<'static, PIO0, 0, DMA_CH0>>;

pub type NetworkRunner = embassy_net::Runner<'static, cyw43::NetDriver<'static>>;

impl<'a: 'static> Cyw43<'a, Uninitialized<'a>> {
    pub async fn new(config: NetworkConfig) -> (Cyw43<'a, Uninitialized<'a>>, WiFiRunner) {
        ...
        let pwr = Output::new(config.pwr_pin, Level::Low);
        let cs = Output::new(config.cs_pin, Level::High);
        let mut pio = Pio::new(config.pio, Irqs);
        let spi = PioSpi::new(
            &mut pio.common,
            pio.sm0,
